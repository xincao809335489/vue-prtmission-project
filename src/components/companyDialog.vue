<template>
  ...
  >
    <el-form :model="company" ref="dialogForm" :rules="companyRules" label-width="80px">
...
export default {
  ...
  methods: {
    resetForm () {
      this.company = Object.assign({}, this.companyRow)
      this.company.options = getRegionData()
    },
    // 转化省市区编码为汉字
    changeProvice (val) {
      this.company.addressName = getCodeToText(val)
    },
    // 确定事件
    companyCompleted () {
      this.$refs.dialogForm.validate(valid => {
        if (valid) {
          // 传递数据更新父组件的对象的对象数据
          const newObj = JSON.parse(JSON.stringify(this.company))
          this.$emit('update:companyRow', newObj)
